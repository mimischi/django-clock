...
class ClockInForm(forms.Form):
    ...
    started = forms.DateTimeField(input_formats=settings.DATETIME_INPUT_FORMATS)
    contract = forms.ModelChoiceField(
        queryset=Contract.objects.none(), empty_label=_("None defined"), required=False
    )
    ...
    def clean(self):
        ...
        if Shift.objects.filter(employee=self.user, finished__isnull=True):
            raise forms.ValidationError(_("You cannot clock into two shifts at once!"))
    ...
    def clock_in(self):
        ...
        Shift.objects.create(started=started, contract=contract, employee=self.user)


class ClockOutForm(forms.Form):
    """Clock out the user."""
    finished = forms.DateTimeField(input_formats=settings.DATETIME_INPUT_FORMATS)
    ...
    def clean(self):
        ...
        if self.spills_into_next_day():
            ...
            if next_day_finished > reference_next_day:
                next_day_finished = timezone.make_aware(
                    timezone.datetime(
                        reference_next_day.year,
                        reference_next_day.month,
                        reference_next_day.day,
                        23,
                        55,
                    )
                )
            ...
            cleaned_data["finished"] = timezone.make_aware(
                timezone.datetime(
                    self.instance.started.year,
                    self.instance.started.month,
                    self.instance.started.day,
                    23,
                    55,
                    00,
                )
            ...
            next_day_started = timezone.make_aware(
                timezone.datetime(
                    next_day_finished.year,
                    next_day_finished.month,
                    next_day_finished.day,
                    0,
                    0,
                )
            )
        ...
        # minutes.
        self.instance.duration = cleaned_data["finished"] - self.instance.started
    ...
    def spills_into_next_day(self, started=None, finished=None):
        ...
        return not (
            (started.year == finished.year)
            and (started.month == finished.month)
            and (started.day == finished.day)
        )
...
class ShiftForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["contract"].queryset = Contract.objects.filter(employee=self.user)
        ...
        self.helper.layout = Layout(
            Field("started", template="shift/fields/datetimepicker_field.html"),
            Field("finished", template="shift/fields/datetimepicker_field.html"),
            Field("contract"),
            Field("reoccuring"),
    ...
    def clean(self):
        ...
        if not (
            (self.started.year == self.finished.year)
            and (self.started.month == self.finished.month)
            and (self.started.day == self.finished.day)
        ):
            self.add_error("finished", _("A shift can never end on the next day."))

        reoccuring = self.cleaned_data.get("reoccuring")
        if (
            reoccuring != "ONCE"
            and self.cleaned_data.get("contract").end_date
            and (
            ...
            self.add_error(
                "end_date", _("You cannot plan shifts after the end of a contract.")
            )
    ...
    def save(self, commit=True):
        ...
        if reoccuring != "ONCE":
            ...
            end_date_datetime = datetime.combine(data["end_date"], datetime.min.time())
            dates = list(
                rrule(
            ...
            for date in dates[1:]:
                data["started"] = timezone.datetime(
                    date.year, date.month, date.day, started.hour, started.minute
                )
                data["finished"] = timezone.datetime(
                    date.year, date.month, date.day, finished.hour, finished.minute
                )
                form = ShiftForm(
                    data=data, **{"user": self.user, "contract": data["contract"]}
                )
                if form.is_valid():
                    form.save()
    ...
    def is_too_long(self, worked_hours=None):
        ...
        return (worked_hours + self.instance.duration) > timezone.timedelta(minutes=600)
    ...
    def work_time_current_day(self):
        ...
        work_time = Shift.objects.filter(
            ...
            finished__lte=timezone.make_aware(datetime(year, month, day, 23, 59)),
        )
        if self.instance.pk:
    ...
    def time_validation(self):
        ...
        if self.finished < self.started:
            ...
            self.add_error(None, _("We cannot save a shift that is this short."))
        ...
        for shift in overlaps:
            started = timezone.localtime(shift.started).strftime("%d.%m.%Y %H:%M")
            finished = timezone.localtime(shift.finished).strftime("%d.%m.%Y %H:%M")
