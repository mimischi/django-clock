...
urlpatterns = [
    ...
    url(
        r'^$',
        ShiftMonthContractView.as_view(
            month_format='%m', ),
        name="list"),
    # View to handle all the quick-actions from the dashboard
    url(r'^quick_action/$', shift_action, name='quick_action'),
    # CreateView to add a new shift
    url(r'^new/$', ShiftManualCreate.as_view(), name='new'),
    # UpdateView to update an existing shift
    url(r'^(?P<pk>\d+)/edit/$', ShiftManualEdit.as_view(), name='edit'),
    # DeleteView to delete an existing shift
    url(r'^(?P<pk>\d+)/delete/$', ShiftManualDelete.as_view(), name='delete'),
    ...
    url(
        r'^(?P<year>[0-9]{4})/(?P<month>[0-9]+)/contract/(?P<contract>\d+)/$',
        ShiftMonthContractView.as_view(month_format='%m'),
