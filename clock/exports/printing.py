...
pdfmetrics.registerFont(
    TTFont(
        "OpenSans-Regular",
        os.path.join(
            str(settings.APPS_DIR), "static/common/fonts/opensans/OpenSans-Regular.ttf"
        ),
    )
)
pdfmetrics.registerFont(
    TTFont(
        "OpenSans-Bold",
        os.path.join(
            str(settings.APPS_DIR), "static/common/fonts/opensans/OpenSans-Bold.ttf"
        ),
    )
)
pdfmetrics.registerFont(
    TTFont(
        "OpenSans-Italic",
        os.path.join(
            str(settings.APPS_DIR), "static/common/fonts/opensans/OpenSans-Italic.ttf"
        ),
    )
)
...
class BoxyLine(Flowable):
    ...
    def __init__(self, x=0, y=-15, width=40, height=15, text_label="", text_box=""):
        ...
        self.styles.add(
            ParagraphStyle(name="HeadText", fontName="OpenSans-Regular", fontSize=10)
        )
...
class ShiftExport:
    ...
    def _header_footer(canvas, doc):
        ...
        styles.add(
            ParagraphStyle(name="NormalText", fontName="OpenSans-Regular", fontSize=10)
        )
        styles.add(
            ParagraphStyle(name="BottomText", fontName="OpenSans-Regular", fontSize=8)
        )
        styles.add(
            ParagraphStyle(
                name="Schluessel",
        ...
        styles.add(
            ParagraphStyle(
                name="centered", alignment=TA_CENTER, fontName="OpenSans-Regular"
            )
        )
    ...
    def print_shifts(self):
        ...
        buffer = self.buffer
        doc = SimpleDocTemplate(buffer, pagesize=self.pagesize, title=pdfTitle)
        ...
        elements.append(
            Paragraph(
                ...
                "§ 17 MiLoG<br /><br /> Wichtig:",
                styles["TitleBoldCentered"],
            )
        )
        ...
        elements.append(
            Paragraph(
                ...
                "maßgelblichen Zeitpunkt aufzubewahren.“",
        ...
        if self.context["total_shift_duration"] > timedelta(seconds=0):
            total_shift_duration = format_dttd(
        ...
        shift_table = Table(
            table_data,
            colWidths=(22.5 * mm, 22.5 * mm, 27.5 * mm, 22.5 * mm, 45 * mm, 12.5 * mm),
        )
