...
class ContractFormTest(TestCase):
    ...
    def test_can_create_contract_without_start_end_date(self):
        form = ContractForm(data={"department": "GU", "hours": 20}, user=self.user)
    ...
    def test_cannot_create_contract_only_with_start_or_end_date(self):
        ...
        form = ContractForm(
            data={"department": "GU", "hours": 40, "start_date": "01.04.2018"},
            user=self.user,
        )
        assert not form.is_valid()
        ...
        form = ContractForm(
            data={"department": "GU", "hours": 40, "end_date": "30.04.2018"},
            user=self.user,
        )
        assert not form.is_valid()
